...
{
    unsigned long curr_time = global_time; //daisy::System::GetUs();
...
{
    double sound = 0.0;
    uint32_t curr_time = global_time; //::System::GetUs();
    double elapsed_time;
    note_t curr_note;
    for(auto iter = pressed_keys.begin(); iter != pressed_keys.end(); iter++)
    {
        curr_note = iter->first;
        elapsed_time = static_cast<double>(curr_time - key_press_time.at(curr_note)) * 0.0001;
